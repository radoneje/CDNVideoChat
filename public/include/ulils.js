...
let dislikeChat=async function(item){
    if(!localStorage.getItem("chatdislike"+item.id)) {
        item.dislikes++;
        await axios.post("/in/api/chatdislike", {id: item.id})
        ...
        await axios.post("/in/api/chatdislike", {id: item.id, undo:1})
    }
...
let likeChat=async function(item){
    if(!localStorage.getItem("chatlike"+item.id)) {
        item.likes++;
        await axios.post("/in/api/chatlike", {id: item.id})
        ...
        await axios.post("/in/api/chatlike", {id: item.id, undo:1})
...
let reqUser=async function (callBack){
    ...
    let register=async ()=>{
        ...
        this.userError=null;
        let r=await axios.post("/in/api/regUser", {id:this.id, name:this.user.name});
...
let chatSend=async function(prm){
    ...
    let r=await axios.post("/in/api/chat",{id:_this.id,text:_this.chatText,userid:_this.user.id})
    _this.chatText="";
...
let dislikeQ=async function(item){
    if(!localStorage.getItem("qdislike"+item.id)) {
        item.dislikes++;
        await axios.post("/in/api/qdislike", {id: item.id})
        ...
        await axios.post("/in/api/qdislike", {id: item.id, undo:1})
    }
...
let likeQ=async function(item){
    if(!localStorage.getItem("qlike"+item.id)) {
        ...
        await axios.post("/in/api/qlike", {id: item.id, undo:1})
...
let qSend=async function(prm){
    ...
    let r=await axios.post("/in/api/q",{id:_this.id,text:_this.qText,userid:_this.user.id})
    _this.qText="";
    _this.q.push(r.data);
    setTimeout(function () {
...
let addImage= async function(sect, _this){
   ...
   elem.onchange=(e)=>{
       ...
       xhr.open("POST", "/in/api/"+sect.toLowerCase()+"File")
